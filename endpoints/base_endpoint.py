...
class Endpoint:
    ...
    def check_body_meme(self, correct_body):
        with allure.step(f"Проверяем структуру ответа"):
            ...
            assert js['updated_by'] == "tot", f"Поле text содержит не те данные что отправили\n{js}"
    ...
    def check_meme_no_additional_field(self):
        js = self.response.json()
        assert "TEST" not in js, "У мема появилось дополнительное поле"
