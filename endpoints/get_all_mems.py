...
class GetAllMems(Endpoint):
    ...
    def get_all_mems_post_negative(self):
        self.response = requests.post(f"{self.BASE_URL}/meme", headers=self.AUTH_TOKEN)
        return self.response

    @allure.step("Проверяем время ответа при запросе всех мемов")
    def get_all_mems_time_response(self):
